...
def render(current_location):# VIEW
	print("Welcome! You are at the " + current_location["name"]) #Where you are 
	print(current_location["cleanText"]) #Description

def get_input():
	choice = input("What is your choice? ") #just takes in choice for action (update) MODEL
	return choice.upper().strip()

def update(current_location, location_label, response): #CONTROLLER
	if response == "":
